...
const SearchPage = ({ params: { id } }: Props) => {
  ...
  const deleteButton = (
    <button onClick={handleDelete} className="bg-indigo-600 text-white px-4 py-2 rounded-lg">
      Delete Search
    </button>
  );

  if (loading) {
    return <h1 className="text-center p-10 animate-pulse text-xl text-indigo-600/50">Loading results...</h1>;
  }
